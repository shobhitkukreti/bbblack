...
static int  pwm_probe(struct platform_device *pdev)
{
	...
	if (!(request_mem_region(epwm.r_mem->start, resource_size(epwm.r_mem),
		"WS2812B")))
		pr_err("%s, EHRPWM Memory Used by
				Another Driver\n", __func__);

	if (!(request_mem_region(CM_PERS_REGBASE, CM_PERS_REGLEN,
		"PWM-CLOCK-Enable")))
		pr_err("%s, CM PERS REG  Memory Used
				by Another Driver\n", __func__);

	if (!(request_mem_region(CTRL_BASE, 4, "Control_Module")))
		pr_err("%s, Control Module  Memory
			Used by Another Driver\n", __func__);

	epwm.ctrl = ioremap_nocache(CTRL_BASE, 4);
	epwm.pwm_base = ioremap_nocache(epwm.r_mem->start,
			resource_size(epwm.r_mem));
...
//	status = pwmss_submodule_state_change(pdev->dev.parent, 0x08);
	...
	pr_info("CLK_Status:%x\n", status);
	status = ioread32(pwmss->pwmss_base + 0x4);

	wmb();// Writing to HW Registers
	pr_info("SYSCONFIG:%x\n", status);
