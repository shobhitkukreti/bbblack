...
unsigned int rx_hook_func(void *priv, struct sk_buff *skb,
		const struct nf_hook_state *state)
{

	pr_info("Incoming Packet on Inf: %s\n", state->in->name);
	return ret;
}

unsigned int tx_hook_func(void *priv, struct sk_buff *skb,
		const struct nf_hook_state *state)
{

	pr_info("Outgoing Packet on Inf: %s\n", state->out->name);
	return ret;
}
