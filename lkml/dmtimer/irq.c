...
static void dmtimer_irqmask(struct irq_data *d)
{
	...
	struct bb_irq_chip *tmp_irq_chip = container_of(chip,
			struct bb_irq_chip, chip);
...
static void dmtimer_irq_alloc_chip(void)
{
		...
		// Hardcoded 4 VIRQ Lines
		for (i = 0; i < 4; i++) {
			irq_set_chip(timer_irq_chip.base + i,
				&timer_irq_chip.chip);
			irq_set_handler(timer_irq_chip.base + i,
				&handle_simple_irq);
			irq_modify_status(timer_irq_chip.base + i,
