...
static int dmtimer_probe(struct platform_device *pdev)
{
	...
	pr_info("Start: %x END:%x , len:%x\n", (u32)r_mem->start,
			(u32)r_mem->end, (u32)r_mem->end-r_mem->start + 1);
#endif


	/* Confirm if IO Memory is available before remapping to virtual space*/

	if ((request_mem_region(CM_PERS_REGBASE, CM_PERS_REGLEN,
		"CM_PERIPHERAL")) == NULL) {
	...
	dmtimer_drv.enable_base_addr = ioremap_nocache(CM_PERS_REGBASE,
			CM_PERS_REGLEN);
	dmtimer_drv.timer7_base = ioremap_nocache(DMTIMER7_REGBASE,
			DMTIMER7_REGLEN);
...
#ifndef SINGLE
	ret = request_irq(dmtimer_drv.virq, (irq_handler_t)dmtimer_irq_handler,
		IRQF_TIMER, "DMTIMER7_IRQ_HANDLER", NULL);
#endif
	...
	ret = request_threaded_irq(nested_irq, NULL,
			(irq_handler_t)&timer_irq_handler, IRQF_ONESHOT,
	...
	iowrite32(0x02, dmtimer_drv.enable_base_addr);
	// enable L4LS_CLKSTCTRL
	iowrite32(30002, (dmtimer_drv.enable_base_addr + 0x7c));
	//enable timer 7 block
	wmb();
	pr_alert("DMTIMER7 Module Status: %x\n",
			ioread32(dmtimer_drv.enable_base_addr + 0x7c));
	pr_alert("DMTIMER7 Clocl L4LS Gate State :%x\n",
			ioread32(dmtimer_drv.enable_base_addr));
	iowrite32(0x02, dmtimer_drv.timer7_base + 0x2c);
	// enable timer 7 over flow intrpt
	iowrite32(0xFE91C9FF, dmtimer_drv.timer7_base + 0x3c);
	// write init value to tclr reg
	iowrite32(0xFE91C9FF, dmtimer_drv.timer7_base + 0x40);
	// put auto reload value 0x00 in TLDR reg
	iowrite32(0x03, dmtimer_drv.timer7_base + 0x38);
	// auto reload mode, enable timer 7

	dmtimer_drv.pdev = pdev;

	#ifdef DEBUG
	dmtimer_drv.my_thread = kthread_run(read_timer,
			(void *)NULL, "TIMER7-Read");
	#endif
...
dmtimer_irq_handler (unsigned int irq, void *dev_id, struct pt_regs *regs)
{
	...
	iowrite32(0x30002, (dmtimer_drv.enable_base_addr
				+ 0x7c));
	iowrite32(0x02, dmtimer_drv.timer7_base
			+ 0x30); // disable timer 7 interrupt
	iowrite32(0x02, dmtimer_drv.timer7_base
			+ 0x28); // clear timer 7 int pending bit by writing 1
	iowrite32(0x02, dmtimer_drv.timer7_base
			+ 0x2c); // enable timer 7 over flow intrpt
	tasklet_schedule(&mytsk); //schedule the tasklet
	handle_nested_irq(timer_irq_chip.base
			+ 2);
	return IRQ_HANDLED;
...
static int read_timer(void *data)
{
	...
	while (dmtimer_drv.enable_base_addr && dmtimer_drv.timer7_base) {
		if (kthread_should_stop())
			break;
		pr_info("TCLR:%x\n", ioread32(dmtimer_drv.timer7_base
					+ 0x38));
		pr_info("TCRR:%x\n", ioread32(dmtimer_drv.timer7_base
					+ 0x3C));
		pr_info("TINT7-EN:%x\n", ioread32(dmtimer_drv.timer7_base
